...
Summarize median auROCs by model parameters -- what is the median auROC summarized across folds where each particular hyperparameter and resampling combination was evaluated? Evaluate how well each hyperparameter configuration performs.
```{r}
metrics_avg <- results_all |> 
  filter(feature_set == "full") |> 
  group_by(hp1, hp2, hp3, resample) |> 
...
Summarize by outer fold -- what is the top performing configuration across each outer fold? Evaluate which hyperparameter configuration "wins" in each outer fold.
```{r}
results_all |> 
  filter(feature_set == "full") |> 
  group_by(outer_split_num, hp1, hp2, hp3, resample) |> 
   summarize(median_roc_auc = median(roc_auc),
             n_folds = n(), .groups = "drop") |> 
  relocate(n_folds) |>
