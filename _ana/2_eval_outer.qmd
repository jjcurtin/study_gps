...
fit_predict_eval <- function(split_num, splits, configs_best){
  ...
  prevalence_in <- mean(d_in$y == "lapse")
  ...
  config_best <- configs_best |> 
    ...
    # KLUDGE!!!!!
    mutate(algorithm = "xgboost2", # MAKE SURE TO UPDATE
           feature_set = batch)
  ...
  metrics_out <- cm |> 
    ...
    suppressWarnings() |>  # warning not about metrics we are returning; check if still needed
    bind_rows(roc) |> 
  ...
  if (batch != "baseline") {
    ...
    # # logistic calibration - SMOOTHING
    # logi_smooth <- predict(model_cal, feat_cal_out,
    #                type = "prob") |>
    #   mutate(truth = feat_cal_out$y) |>
    #   cal_estimate_logistic(truth = truth,
    #                          estimate = starts_with(".pred_"),
    #                          smooth = TRUE)
    # preds_prob_logi_smooth <- preds_prob |>
    #   cal_apply(logi_smooth)
    # # 
    # # # logistic calibration - NO SMOOTHING
    # logi_no_smooth <- predict(model_cal, feat_cal_out,
    #                type = "prob") |>
    #   mutate(truth = feat_cal_out$y) |>
    #   cal_estimate_logistic(truth = truth,
    #                          estimate = starts_with(".pred_"),
    #                          smooth = FALSE)
    # preds_prob_logi_no_smooth <- preds_prob |>
    #   cal_apply(logi_no_smooth)
    ...
    probs_out <- tibble(id_obs = d_out$id_obs,
                        ...
                        prev_inner = rep(prevalence_in, nrow(preds_prob)),
                        #prob_iso = preds_prob_iso[[str_c(".pred_", y_level_pos)]],
                        #prob_logi_smooth = preds_prob_logi_smooth[[str_c(".pred_", y_level_pos)]],
                        #prob_logi_no_smooth = preds_prob_logi_no_smooth[[str_c(".pred_", y_level_pos)]],
...
metrics_avg <- metrics_raw |>
  filter(feature_set == batch) |> # full or baseline
  group_by(hp1, hp2, hp3, resample, outer_split_num) |>
   summarize(across(c(roc_auc, mn_log_loss, accuracy, bal_accuracy, sens, spec, ppv, npv),
                     median),
              n_jobs = n(), .groups = "drop") |> 
  relocate(outer_split_num, n_jobs) |> 
  arrange(outer_split_num, desc(roc_auc)) # set to either mn_log_loss from roc_auc -- using roc_auc now
                                              # note that log loss needs to be ASCENDING
                                              # while roc_auc needs to be DESCENDING
```
...
configs_best <- metrics_avg |> 
  ...
  slice(1) |> 
  ungroup() #|> 
  #relocate(roc_auc, .before = mn_log_loss)
...
configs_best |> pull(mn_log_loss) |> mean()
configs_best |> pull(mn_log_loss) |> median()
```
...
if(!file.exists(here::here(path_models, str_c("outer_metrics_", 
  ...
  d <- arrange(.data = d, subid, dttm_label) |>
    format_data(d) |>
    #arrange(label_num) |> cp note: do we need this??; do not have currently -- think adding above arrange will correct
  ...
  write_csv(tibble(stage = "metrics_save",
                   outer_split_num = NA, 
                   start_time = Sys.time()),
            here::here(path_models, str_c("tmp_metrics_outer_progress_",
  ...
  if (batch != "baseline") { # need to address this in less kludgy way
