...
## Desicsions about next points far away

## Decisions about speed

# Decisions about long time at point


## Decisions about one observation per day points
...
Filtering
```{r}
# implement filters separately for readability
enriched <- enriched |> 
  # 500m is ~.3 of a mile
  # filter out if the duration between points is greater than 5 min
  # and the distance traveled is more than 500m
  filter(!(duration > 5 & dist > 500)) |> # filter out improbable jumps
         # calculate speed in km/h, convert to mph
  mutate(speed = ((dist / 1000)/(duration / 60) * 0.621371),
         # assume at known location if within 50m
         known_loc = if_else(dist_context <= 50 & speed < 4, TRUE, FALSE),
         # set points at or below 1 km/h (~0.62mph) to be stationary
...
```{r}
speed_dup <- function(enriched) {
  inds <- which(enriched$dist == 0 & enriched$duration == 0)
  rows <- lapply(inds, function(x) (x-1):(x+1))
  enriched[unlist(rows),]
}
```
...
Count number of is.na(speed)
```{r}
sum(is.na(enriched$speed)) # 32553
```

Percentage of points above threshold to see if they are
at the beginning or end of a series (want most points to fall in middle).
```{r}
enriched_sub <- enriched |>
  filter(known_loc == 1) |>
  select(subid, sgmnt_type, track_type, duration, dist, speed, context_id) |>
  # if speed is greater than amount, it will be cut for place analyses
  # average walking speed is about 3 miles per hour
...
first_seq_ids <- which(enriched_sub$row_seq == 1)
last_seq_ids <- first_seq_ids - 1

enriched_sub <- enriched_sub |>
  mutate(row_place = if_else(row_number() %in% first_seq_ids, "first", "0"),
         row_place = if_else(row_number() %in% last_seq_ids, "last", row_place),
         row_place = if_else(row_place=="0", "middle", row_place))
         # another thing to do would be to ignore middle rows?
         # some rows listed as middle might be just second to last
         # other option -- classify some of these points as transition?
         # stationary --> transition --> transit?

enriched_sub |>
  janitor::tabyl(cut, row_place) |> 
...
enriched |>
  ...
  # separately for moves and followmee
  filter(duration > 121) |>
...
enriched_obs |> filter(n_obs == 1) |> nrow()
