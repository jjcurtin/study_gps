...
enriched |>
  ...
  mutate(known_loc = if_else(dist_context <= 50, 1, 0)) |> 
  pull(known_loc) |> 
  mean()
...
enriched |> 
  ...
  mutate(speed = dist/duration,
         known_loc = if_else(dist_context <= 50, 1, 0)) |> 
  filter(speed < 100) |>  # what are these 40k points
  pull(known_loc) |> 
  sum()
  
enriched |> filter(dist_context <= 50) |> mutate(speed = dist/duration) |> select(subid, context_id, speed) |> view()
  
  filter(dist > 3000) |> nrow() 
  ggplot(aes(x = dist)) +
