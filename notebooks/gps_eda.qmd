...
path_notes <- format_path("analysis_risk/shared/notes")
```
...
visit_dates <- read_csv(here::here(path_gps, "study_dates.csv"), show_col_types = FALSE) |> 
  glimpse()
...
If there is no GPS data collected on a particular day, that date (i.e., row) will not exist.
```{r}
