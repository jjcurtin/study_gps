...
> CP REGENERATE THESE TMRW ON 7/15!!
...
```{r}
gps_162 <- gps |> filter(subid == 162)

cbbox <- make_bbox(lon = gps_162$lon, lat = gps_162$lat, f = .1)
sq_map <- get_map(location = cbbox, maptype = "stamen_toner_lite")
    
p <- ggmap(sq_map) + 
  geom_point(data = gps_162, aes(x = lon, y = lat),
...
pdf(here::here(path_maps, "162_by_day.pdf"))
for(i in 1:92){
  print(ggmap(sq_map) + 
  geom_point(data = gps_162, aes(x = lon, y = lat),
             size = 1, color="darkred") +
  labs(x = " ", y = " ", title = "Sub 162") +
  theme_minimal() +
