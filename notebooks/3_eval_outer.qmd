...
preds_out <- read_csv(here::here(path_models, str_c("inner_metrics_", version, "_", 
                                                    cv, "_", model, ".csv")))
metrics_out <- read_csv(here::here(path_models, str_c("inner_metrics_", version, "_", 
                                                    cv, "_", model, ".csv")))
```
...
> CP: why is this lower than what it looked like when I ran the inner metrics file?
...
metrics_out |> 
  ...
  summarize(median(roc_auc), mean(roc_auc), min(roc_auc), max(roc_auc), sd(roc_auc))
```
...
> CP: it looks like I need to manually add back in true lapse/no lapse outcomes?
