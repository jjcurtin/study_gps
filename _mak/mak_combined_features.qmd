...
Get rid of other features (e.g. no) that aren't useful, and remove Chris' features. CP commenting out for now, think we want everything.
```{r}
#gps <- gps |>
  # not useful
  #select(-contains(".no")) |> 
...
features_to_act <- c("evening_out", "evening_out.no",
                     "home", "home.no", "location_var")

gps <- gps |> 
  rename_with(
    ~ sub(
        # Match: last segment before optional .no
        paste0("(", paste(features_to_act, collapse = "|"), ")$"),
        # Replace with: act_ + matched segment
        "act_\\1",
        .x
    ),
    starts_with("p")
