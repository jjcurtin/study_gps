...
fit_predict_eval <- function(outer_split_num = NULL, inner_split_num = NULL, split_num, splits, configs_best){
  ...
  #d_in <- training(splits$splits[[outer_split_num]]) |>
    select(-id_obs)  # not used for training; only needed in d_out to tag for later joins 
  d_out <- testing(splits$splits[[split_num]])
  #d_out <- testing(splits$splits[[outer_split_num]])
  
  # this needs to be ONE ROW
  config_best <- configs_best |>
    slice(1) |> # claire added
    #slice(split_num) |>
    #slice(outer_split_num) |> 
    rename(accuracy_in = accuracy, #n_jobs_in = n_jobs,
...
> CP: I actually think this was already done with the loaded in file so I commented it out

Average metrics for each configuration across inner folds for each outer fold
```{r}
# metrics_avg <- best_config |> 
#   group_by(algorithm, feature_set, hp1, hp2, hp3, resample, outer_split_num) |> 
#   #summarize(across(c(accuracy, bal_accuracy, roc_auc, sens, spec, ppv, npv), relocate(outer_split_num))) |>
#   arrange(outer_split_num, desc(roc_auc))
```

Best configuration for each outer fold

> Should this be across each outer fold (1:30), or across all splits (1:300)?

```{r}
configs_best <- best_config |> 
  group_by(outer_split_num) |> # maybe need different grouping?
...
```{r}
if(!file.exists(here::here(path_models, str_c("inner_metrics_", 
                                  window, "_", version, "_", 
                                  cv, "_", model, ".rds")))){ 
  ...
  # 
  # all <- map2(inner_split_num_list, outer_split_num_list, \(inner_split_num_list, outer_split_num_list)
  #      fit_predict_eval(inner_split_num = inner_split_num_list, outer_split_num = outer_split_num_list,
  #                       splits = splits, configs_best)) |>
  #   bind_rows()
  ...
  metrics_out <- all |> 
    ...
    list_rbind() |> 
    write_rds(here::here(path_models, str_c("inner_metrics_", 
  ...
  metrics_out <- read_rds(here::here(path_models, str_c("inner_metrics_", 
                                                       window, "_", 
                                                       version, "_", 
                                                       cv, "_", model, ".rds")))
