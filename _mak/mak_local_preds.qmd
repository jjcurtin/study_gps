...
fit_predict <- function(split_num, splits, config_best){
  ...
  d_in <- splits$splits[[split_num]] |>
    ...
    #mutate(label_num = 1:n(), # add fake label_num because build_recipe expects it, removed immed.
           #cv_strat = strat) # should update build_recipe to look for strat, not cv_strat
...
config_best <- 
  ...
  mutate(n = n(),
         algorithm = "xgboost") |> 
  slice(1) |> 
  select(feature_set, hp1, hp2, hp3, resample, n, algorithm) |>
