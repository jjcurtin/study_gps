...
```{r}
# Define the beta calibration function
beta_function <- function(mh, a, b, inter) {
  b * log(1 - mh) - a * log(mh) - inter
}

# Evaluate over a fine grid in (0, 1)
mh_values <- seq(1e-6, 1 - 1e-6, length.out = 1000)
f_values <- beta_function(mh_values, a, b, inter)

# Plot the function
plot(mh_values, f_values, type = "l", col = "blue", lwd = 2,
     main = "Beta Calibration Function",
     xlab = "mh", ylab = "f(mh)")
abline(h = 0, col = "red", lty = 2)  # Add horizontal line at 0

# Check specific points
points_to_check <- c(0.01, 0.5, 0.99)
values_at_points <- beta_function(points_to_check, a, b, inter)

cat("Function values at specific mh points:\n")
for (i in seq_along(points_to_check)) {
  cat(sprintf("f(%.2f) = %.4f\n", points_to_check[i], values_at_points[i]))
}

# Check if the function crosses zero
signs <- sign(f_values)
sign_change <- any(diff(signs) != 0)

if (sign_change) {
  message("✅ Function crosses zero: uniroot() would succeed.")
} else {
  message("❌ Function does not cross zero: uniroot() would fail.")
}
```

## Plot raw probabilities of failed folds

Modified fit_predict().
```{r}
...
cal_raw |> 
  ...
  ggplot(data = _, aes(x = midpoints, y = mean_lapse, color = probs)) +
    ...
    geom_point() +
    labs(title = "Split 10",
         x = "Predicted Lapse Probability (bin mid-point)",
