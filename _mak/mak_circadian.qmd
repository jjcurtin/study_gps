...
## Load tidy and establish conflicts policy
...
Let's check and see the number of unique context IDs per subject.
```{r}
gps_centroid |> 
  group_by(subid) |> 
  summarise(count = n_distinct(context_id)) |> 
  pull(count) |> 
  hist()
...
```{r}
cm_windows <- gps_centroid$subid |> 
  unique() |> 
  furrr::future_map(\(subid) cm_window(df = gps_centroid,
                                 subid_arg = subid,
                                 time_col = "time_hr",
                                 date_col = "time")) |>
  bind_rows()
...
cm_results <- gps_centroid$subid |>
  unique() |> 
  furrr::future_map(\(subid) cm_slide(gps_df = gps_centroid,
