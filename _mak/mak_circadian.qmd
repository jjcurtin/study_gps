...
Optional load in of things calculated below for easy review!
```{r}
cm_windows <- read_csv(here::here(path_gps, "gps_cm_windows.csv"), 
                 show_col_types = FALSE) |> 
  mutate(time_start = with_tz(time_start, "America/Chicago"),
         time_end = with_tz(time_end, "America/Chicago"))
  
cm_results <- read_csv(here::here(path_gps, "gps_cm.csv"), 
                 show_col_types = FALSE) |> 
    mutate(time_start = with_tz(time_start, "America/Chicago"),
         time_end = with_tz(time_end, "America/Chicago"))
```
...
We could also create clusters (below I am using Chris Janssen's clustering code). Note that context_id numbers repeat. Every subject's cluster counting restarts from 1 in Chris' code so there are multiple 1s (for example), but each is a DIFFERENT location relative to that person's own data.
```{r}
...
```{r}
gps_simple <- gps |> 
  select(subid, lat, lon, dist, duration, speed, time, time_hr)
```
...
if (file.exists(here::here(path_gps, "gps_clustered.csv"))) {
  ...
  gps_clustered <- subid_all |> 
    ...
    bind_rows() |>
    mutate(at_cluster = if_else(dist_context <= 50 & speed <= 4,
                                TRUE, FALSE),
...
```{r}
cm_slide <- function(gps_df, window_df, subid_arg, time_col = "time_hr", date_col = "time") {
  
  gps_df_tmp <- gps_df |>
    filter(subid == subid_arg) |>
    arrange(.data[[time_col]])
  
  window_df_tmp <- window_df |> filter(subid == subid_arg)
  
  # calculate CM for each sliding window
  results <- purrr::pmap_dfr(window_df_tmp, \(subid, time_start, time_end, 
                                             window_start, window_end, ...) {
    
    df_window <- gps_df_tmp |> 
      filter(
...
Now let's first create the windows for all of our subjects.
```{r}
cm_window <- function(df, subid_arg, time_col = "time_hr", date_col = "time", window = 168, step = 24) {
  ...
  max_time <- max(df_tmp[[time_col]], na.rm = TRUE)
  
  
  window_starts <- seq(0, max_time - window, by = step)
  ...
  else min_date + dhours(start_time)
  ...
  tibble(
    subid = subid_arg,
    time_start = floor_date(date_start_val, "day"),
    time_end   = ceiling_date(date_end_val, "day") - seconds(1),
...
Create windows for each subject, starting from midnight on their second day of study participation.
```{r}
cm_windows <- gps_clustered$subid |> 
  unique() |> 
  furrr::future_map(\(subid) cm_window(df = gps_clustered,
                                 subid_arg = subid,
                                 time_col = "time_hr",
                                 date_col = "time")) |>
  bind_rows()
...
cm_results <- gps_clustered$subid |>
  unique() |> 
  furrr::future_map(\(subid) cm_slide(gps_df = gps_clustered,
                                      window_df = cm_windows,
                                 subid_arg = subid,
                                 time_col = "time_hr",
                                 date_col = "time")) |>
  bind_rows()
...
cm_results |> 
  filter(!is.na(cm)) |> 
  summarise(
    n_negative = sum(cm < 0), # come back and check this, should this be -1?
    n_positive = sum(cm > 0),
    n_zero = sum(cm == 0)
  )
```
...
128 looks like someone who has a good amount of data but lots of irregular patterns (i.e., low CM values).

```{r}
cm_results |> 
  filter(subid == 128) |> 
  filter(as.Date(time_start) >= as.Date("2018-10-02") &
           as.Date(time_start) <= as.Date("2018-10-10"))
```

Check proportion of time spent at known clusters relative to not in this period.
```{r}
gps_clustered |> 
  filter(subid == 128) |> 
  filter(as.Date(time) >= as.Date("2018-10-02") &
           as.Date(time) <= as.Date("2018-10-16")) |>
  summarize(dur_known = sum(duration[at_cluster], na.rm = TRUE),
            dur_unknown = sum(duration[!at_cluster], na.rm = TRUE))
  
```

Check number of times went to different clusters in this period.
```{r}
gps_clustered |> 
  filter(subid == 128) |> 
  filter(as.Date(time) >= as.Date("2018-10-02") &
           as.Date(time) <= as.Date("2018-10-16")) |>
  filter(at_cluster == TRUE) |> 
  count(context_id, name = "n_visits")
```

Create a plot displaying an actogram (raw points by day, with a dot for median time), daily median visit times to clusters, and sliding CM values.
```{r}
# filter down to sub and convert time for plotting
gps_sub <- gps_clustered |> 
  filter(subid == 128, at_cluster == TRUE,
         as.Date(time) >= as.Date("2018-10-02") &
         as.Date(time) <= as.Date("2018-10-16")) |> 
  mutate(
    date = as.Date(time),
...
# get median visit time
daily_medians <- gps_sub |>
  group_by(date) |>
  summarise(
...
# filter cm data and format dates for plots
cm_sub <- cm_results |> 
  filter(subid == 128 &
         as.Date(time_start) >= as.Date("2018-10-02") &
           as.Date(time_start) <= as.Date("2018-10-10")) |> 
  mutate(time_start = as.Date(time_start),
         time_end   = as.Date(time_end))

# plot all raw points as an "actogram"
p_actogram <- ggplot(gps_sub, aes(x = hour, y = date, color = factor(context_id))) +
  geom_point(size = 2, alpha = 0.7) +
    geom_point(
    data = daily_medians,
    aes(x = median_hour, y = date), # big black dot for median time!!
    color = "black",
    size = 4
  ) +
  ...
  labs(
    ...
    y = "Date",
    color = "Cluster",
    title = paste("Actogram - Subject 128")
  ) +
   scale_y_date(
    date_breaks = "1 day",
    date_labels = "%b %d"
...
# plot median visit times
p_medians <- ggplot(daily_medians, aes(x = date, y = median_hour)) +
  ...
  labs(
    x = NULL,
    y = "Median Hour",
    title = "Daily Median Visit Times"
  ) +
  scale_x_date(
    date_breaks = "1 day",
    date_labels = "%b %d"
...
# plot cm values over time w end date of window as the reference date
# remember that each value is calculated based on 7 preceding days!!
p_cm <- ggplot(cm_sub, aes(x = time_end, y = cm)) +
  geom_line(color = "black", size = 1) +
  geom_point(color = "black", size = 2) +
  labs(
    x = "End Date of Window",
    y = "CM",
    title = "Sliding-Window Circadian Movement (CM)"
  ) +
   scale_x_date(
    date_breaks = "1 day",
    date_labels = "%b %d"
...
# line panels up
p_actogram / p_medians/ p_cm + plot_layout(heights = c(3, 1, 1))
```

### Regular rhythm

11 looks like someone who has very regular patterns in the middle of study participation (i.e., high CM values).

```{r}
cm_results |>
  filter(subid == 11) |> 
  filter(as.Date(time_start) >= as.Date("2017-08-07") &
           as.Date(time_start) <= as.Date("2017-08-15"))
```
...
gps_clustered |> 
  filter(subid == 11) |> 
  filter(as.Date(time) >= as.Date("2017-08-07") &
           as.Date(time) <= as.Date("2017-08-21")) |>
  summarize(dur_known = sum(duration[at_cluster], na.rm = TRUE),
            dur_unknown = sum(duration[!at_cluster], na.rm = TRUE))
...
```{r}
# filter down clustered data for plotting and convert time for plotting
gps_sub <- gps_clustered |> 
  filter(subid == 11, at_cluster == TRUE,
         as.Date(time) >= as.Date("2017-08-07") &
         as.Date(time) <= as.Date("2017-08-21")) |> 
  mutate(
    date = as.Date(time),
    hour = hour(time) + minute(time)/60
...
# check what the median visit time was for each day
daily_medians <- gps_sub |>
  group_by(date) |>
  summarise(
    median_hour = median(hour, na.rm = TRUE),
...
# filter down our CM data and convert dates for plotting
cm_sub <- cm_results |> 
  filter(subid == 11 &
         as.Date(time_start) >= as.Date("2017-08-07") &
           as.Date(time_start) <= as.Date("2017-08-15")) |> 
  mutate(time_start = as.Date(time_start),
         time_end   = as.Date(time_end))

# create "actogram" plot with markers for median visit times
p_actogram <- ggplot(gps_sub, aes(x = hour, y = date, color = factor(context_id))) +
  geom_point(size = 2, alpha = 0.7) +
  geom_point(
    data = daily_medians,
    aes(x = median_hour, y = date), # dots for visit times
    color = "black",
    size = 4
  ) +
  scale_x_continuous(breaks = seq(0, 24, by = 2)) +
  labs(
    ...
    y = "Date",
    color = "Cluster",
    title = paste("Actogram - Subject 11")
  ) +
   scale_y_date(
    date_breaks = "1 day",
    date_labels = "%b %d"
...
# plot cm by end of window
# these data are created using the previous seven days from the end window
p_cm <- ggplot(cm_sub, aes(x = time_end, y = cm)) +
  geom_line(color = "black", size = 1) +
  geom_point(color = "black", size = 2) +
  labs(
...
```{r}
cm_windows |> 
  write_csv((here::here(path_gps, "gps_cm_windows.csv")))
```
