...
The median auROC across the 300 folds achieved fair performance (Mdn = 0.721, IQR = XX, range = XX-XX). @fig-auroc-histogram displays a histogram of model performance distribution across all folds. A receiver operating characteristic curve is displayed in @fig-auroc-plot, representing aggregate predicted lapse probabilities across all 300 folds of the validation set.

We also used the posterior probability distributions for the auROCs for our best performing validation set model to formally characterize model performance. The median auROC was XX (95% CI [XX-XX]; @fig-pp).
