...
Our day-level model of lapse prediction using geolocation data performs at an "acceptable" threshold [@mandrekarReceiverOperatingCharacteristic2010], suggesting that, while there is still substantial room for improvement in model performance, geolocation data can predict future alcohol lapse in the next day with fair sensitivity and specificity.
...
- All models exhibited differential performance across four broad classes: race/ethnicity (white versus non-white), sex at birth (male versus female), age (younger than 55 or older than or equal to 55), and income (above the poverty line versus below the poverty line).
