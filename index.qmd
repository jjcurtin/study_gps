...
Across fairness subgroup comparisons, we did not find any evidence to suggest that performance differed by sex, income, race/ethnicity, age, or educational attainment (@fig-3). A table of these comparisons can be seen in @tbl-2. <!--CP: LEFT OFF HERE!! MAKE TABLE :^) -->

<!--CP: maybe time to bring this in for log loss?-->
...
The best model configuration produced relatively well-calibrated probabilities which were clustered around the observed lapse rate. We present a calibration plot collapsed across all folds as well as a histogram faceted by lapse event in **INSERT FIGURE (A + B)**. The number of observations in each bin are presented in **INSERT TABLE**. We provide calibration plots and raw probability distributions separated by fold, along with a table displaying the number of observation in each bin, in **FIGURE HERE**, **FIGURE HERE**, and **TABLE HERE** in the supplement.

The median skill score was **INSERT HERE**, suggesting that mean log loss was reduced by **INSERT HERE** in our model relative to one which predicts the baserate of lapse. Histograms displaying the distribution of mean log loss and skill scores across each fold are presented in **FIGURE HERE** and **FIGURE HERE** in the supplement.

### Feature Importance

Global importance (mean absolute Shapley values) for feature categories are displayed in **INSERT FIGURE**. <!--CP: need to think about how we are aggregating across features, probably want to combine them more-->

## Discussion

### Model Performance

Our day-level model of alcohol lapse prediction using geolocation data has a median auROC of **INSERT**. Bayesian model comparisons indicated that this model performed better than both chance and a model which only included demographicv ariables. While not clinically implementable on its own, these findings confirm that there is strong evidence of predictive signal in these data and are an important first step towards an eventual continuous risk monitoring and support system. This study also provided explanatory insights by way of quantifying feature importance as well as a crucial examination of model fairness across classes, detailed in the following sections.
