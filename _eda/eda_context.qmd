...
if(file.exists(here::here(path_shared, "gps_enriched.csv.xz"))){
  ...
  study_dates <- read_csv(here::here(path_gps, "study_dates_gps.csv"),
                          show_col_types = FALSE) |>
...
(See sop_android_tech_setup_followmee in analysis_risk/methods/sop)
...
places |> 
  group_by(subid) |> 
  select(subid, duration_diff) |> 
  unique() |> 
  arrange(desc(duration_diff))
```

Super different for some folks! Maybe due to our 24 hour thresholding? Let's check subid 185. Note to discuss with JC.
```{r}
places |> 
  filter(subid == 185) |> 
